...
public final class FilePersistor {
    ...
    public void persist(Track t) {
        shutupLog();
        try {
            AudioFile f   = AudioFileIO.read(new File(t.location()
                                                       .value()));
            Tag       tag = f.getTag();
            tag.setField(FieldKey.TITLE,
                         t.title()
                          .value());
            if (t.artist()
                 .isPresent()) tag.setField(FieldKey.ARTIST,
                                            t.artist()
                                             .get()
                                             .value());
            if (t.album()
                 .isPresent()) tag.setField(FieldKey.ALBUM,
                                            t.album()
                                             .get()
                                             .value());
            if (t.genre()
                 .isPresent()) tag.setField(FieldKey.GENRE,
                                            t.genre()
                                             .get()
                                             .value());
            if (t.year()
                 .isPresent()) tag.setField(FieldKey.YEAR,
                                            t.year()
                                             .get()
                                             .value());
            if (t.bpm()
                 .isPresent()) tag.setField(FieldKey.BPM,
                                            t.bpm()
                                             .get()
                                             .value()
                                             .toString());
            if (t.key()
                 .isPresent()) tag.setField(FieldKey.KEY,
                                            t.key()
                                             .get()
                                             .value());
            if (t.comments()
                 .isPresent()) tag.setField(FieldKey.COMMENT,
                                            t.comments()
                                             .get()
                                             .value());
