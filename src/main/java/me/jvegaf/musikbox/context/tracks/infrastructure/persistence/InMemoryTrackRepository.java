...
public final class InMemoryTrackRepository implements TrackRepository {
    ...
    @Override public void save(Track track) { tracks.add(track); }

    @Override public void update(Track track) {
        tracks.removeIf(t -> t.id().equals(track.id()));
        tracks.add(track);
    }

    @Override public List<Track> searchAll() {
        return tracks;
    }

    @Override public Optional<Track> find(TrackId id) {
        return tracks.stream().filter(track -> track.id().equals(id)).findFirst();
    }

    @Override public List<Track> matching(Criteria criteria) {
        return null;
    }
}
